...
/*			if (!empty($this->value[1])) {
			...
			//echo '<div class="redux-groups-accordion-group redux-dummy" id="redux-dummy-' . $this->field['id'] . '"><h3><span class="redux-groups-header">' . __("New ", "redux-framework") . $this->field['groupname'] . '</span></h3>';
			...
			$field_is_title = true;
				foreach ($this->field['fields'] as $field) {
					...
					$content = str_replace('name=""', 'data-name=' . $this->args['opt_name'] . '[' . $this->field['id'] . '][@]['.$field['id'].']', $content);
			...
			if (!empty($groups)) {
				foreach ($groups as $group) {
					...
					foreach ($this->field['fields'] as $field) {
						...
						$content = str_replace('name=""', 'name='.$this->args['opt_name'] . '[' . $this->field['id'] . ']['.$x.']['.$field['id'].']', $content);
