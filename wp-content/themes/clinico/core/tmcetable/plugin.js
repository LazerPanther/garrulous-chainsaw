...
define("tinymce/tableplugin/Plugin", [
	...
	function Plugin(editor) {
			...
			editor.windowManager.open({
				...
				onsubmit: function() {
					...
					editor.undoManager.transact(function() {
						if (!tableElm) {
							tableElm = insertTable(data.cols || 1, data.rows || 1, false);
						}
		...
		function insertTable(cols, rows, bIsBlocks) {
			var y, x, html;

			html = bIsBlocks ? '<table id="__mce" width="100%"><tbody>' : '<table id="__mce"><tbody>';

			var td_width = bIsBlocks ? ' style="width:' + (100/cols).toFixed(2) + '%"' : '';

			for (y = 0; y < rows; y++) {
				html += '<tr>';

				for (x = 0; x < cols; x++) {
					html += '<td' + td_width + '>' + (Env.ie ? " " : '<br>') + '</td>';
				}
		...
		function generateTableGrid(c, r) {
			var html = '';

			html = '<table role="grid" class="mce-grid mce-grid-border s' + c + 'x' + r + '" aria-readonly="true">';

			for (var y = 0; y < c; y++) {
				html += '<tr>';

				for (var x = 0; x < r; x++) {
					html += '<td role="gridcell" tabindex="-1"><a id="mcegrid' + (y * c + x) + '" href="#" ' +
		...
		function selectGrid(tx, ty, c, r, control) {
		...
		if (editor.settings.table_grid === false) {
				...
				menu: [
					{
						...
						onmousemove: function(e) {
							...
							if (target.tagName.toUpperCase() == 'A') {
								...
								if (x !== this.lastX || y !== this.lastY) {
									selectGrid(x, y, 10, 10, e.control);
						...
						onclick: function(e) {
							if (e.target.tagName.toUpperCase() == 'A') {
								...
								insertTable(this.lastX + 1, this.lastY + 1, false);
							}
						}
					}
				]
			...
			editor.addMenuItem('insertblocks', {
				text: 'Insert blocks',
				icon: 'table',
				context: 'table',
				ariaHideMenu: true,
				...
				menu: [
					{
						...
						onclick: function(e) {
							if (e.target.tagName.toUpperCase() == 'A') {
								...
								insertTable(this.lastX + 1, this.lastY + 1, true);
							}
						}
					}
				]
		...
		var menuItems = [];
		each("inserttable insertblocks tableprops deletetable | cell row column".split(' '), function(name) {
			if (name == '|') {
				menuItems.push({text: '-'});
			} else {
